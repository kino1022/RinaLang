...
# refキーワード
- 変数を参照として扱うために用いるキーワード
- 参照として扱う変数はmutキーワードを付記することで値の書き換えもすることができます。

# 可変関数
インスタンスのフィールドを書き換える事の出来る引数はmutキーワードを付記する必要があります。
```
fn move (mut ref x: int32, mut ref y: int32, move_value: int32) : void {
    x += move_value;
    y += move_value;
}
```